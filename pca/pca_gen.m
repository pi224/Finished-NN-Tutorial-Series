...
%x = x(:, 1:3000); %make x of smaller size, so that matlab won't explode

%%% YOUR CODE HERE %%%
size(x)

disp('normalizing data to the mean ... ')
for i = 1:size(x, 2)
    x(:, i) = x(:, i) - mean(x(:, i));
end
%%================================================================
...
display('optimizing k vs variance')

it = 0;
eigenSum = sum(D);
k = 1;
while sum(D(1:k))/eigenSum < varRatio
    k = k + 1;
end
...
for i = 1:size(xPCAWhite, 2)
    xPCAWhite(:, i) = xRot(:, i) .* regularizer';
end
